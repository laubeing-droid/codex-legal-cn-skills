...
| 文件 | 说明 |
|------|------|
| install.ps1 | 通用安装：克隆上游、部署技能到所有客户端、委托 MCP 连接器 |
| update.ps1 | 同步上游 + 全环境技能更新 + 委托 MCP 验证 |
| uninstall.ps1 | 一键卸载所有已安装技能和上游缓存 |
| verify.ps1 | 检查所有环境安装完整性 |
| skills/ | 13 个技能入口定义（SKILL.md，供 Codex Desktop 使用） |
| docs/ | 完整中文文档 |
...
`install.ps1` 和 `update.ps1` 均自动委托给该仓库。支持**全环境写入**：
...
快速配置：运行 `update.ps1` 查看凭证状态，或编辑对应客户端的配置文件替换凭证。
